...
Ankiのランチャーはデフォルトで公式のPyQtビルドを使用します。これにより、関連するPython/Qtバージョンを持たないディストリビューションでもAnkiのインストールが容易になりますが、特定のQtテーマやFCITX入力メソッドのサポートなど、Linuxディストリビューションが提供する特定のQt機能にアクセスできない場合があります。

お使いのLinuxディストリビューションが最新のAnkiパッケージを提供している場合は、それを使用するのが最も簡単かもしれません。

提供されていない場合、上級ユーザーはAnkiのランチャーをシステムのQtバージョンと組み合わせることができます。これを行うには、システムにAnkiがサポートするPythonバージョン（まもなく3.11以上）と、適切なPyQtライブラリ（6.2以上）が必要です。

警告：これは実験的な機能であり、システムのQtは一部のバグを修正する一方で、他のバグを導入する可能性があります。

1. Pythonと関連するPyQtパッケージをインストールします。Ubuntuの場合：

   > sudo apt install python3-pyqt6.qtwebengine

1. 以前にランチャーを使用していた場合は、`rm -rf ~/.local/share/AnkiProgramFiles`を実行します。

1. ランチャーをuntarし、そのフォルダにcdします。

1. `touch system_qt`を実行して、そのフォルダにsystem_qtファイルを作成します。

1. ./ankiまたは./install.shでAnkiをインストールします。インストールされたパッケージのリストに、PyQt6の記述がないことを確認してください。
