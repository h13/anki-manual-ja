...
入力した解答でAnkiに文字のアクセントを正解と比較させたくない場合は、フィールドで`type:nc`を使用できます。

    {{type:nc:Front}}

これにより、アクセントの違いがAnkiによって不正解としてマークされなくなります。
例えば、`بطيخ`は`بَطِّيخ`と同じとして扱われ、`elite`は`élite`と同じとして扱われます。